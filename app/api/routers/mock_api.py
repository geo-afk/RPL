...
from app.api.mock_server.rest.response_models import (
    ProjectCreate, ProjectUpdate, TaskResponse, TaskCreate,
    TaskUpdate, ProjectResponse, HealthResponse,
    DeleteResponse, TaskStatusResponse
)


async def ensure_seeded():
    seeded = await seed(get_db())
    if seeded:
...
async def list_projects(db: AsyncSession = Depends(get_db)):
    result = await db.execute(Project.__table__.select().order_by(Project.id))
...
async def update_task_status(task_id: int, task_status: TaskStatus, db: AsyncSession = Depends(get_db)):
    task = await db.get(Task, task_id)
    if not task:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Task not found")
